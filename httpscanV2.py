...
printLock = threading.Semaphore(3)  #lock Screen print
  ...
  def request(self):
    with threading.Lock():
      while self.IPs.qsize() > 0:
        ip = self.IPs.get()
        httpport = {'80':'http','8080':'http','3128':'http','8081':'http','9080':'http','443':'https','8443':'https'}
        for i in httpport.keys():
          try:
            # print(httpport[i]+'://'+str(ip)+i)
            r = requests.Session().get(httpport[i]+'://'+str(ip)+':'+i,headers=header,timeout=TimeOut)
            ...
            printLock.acquire()
            print "|%-20s|%-6s|%-20s|%-30s|" % (ip+':'+i,status,banner,title)
            print "+--------------------+------+--------------------+------------------------------+"
